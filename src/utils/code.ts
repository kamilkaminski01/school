...
export const blogCodeJenkinsCiCdPipeline = [
  {
    lang: CODE_LANG.groovy,
    code: `node {
    try {
        onBuild()
  ...
  {
    lang: CODE_LANG.groovy,
    code: `// Run when stage in build fails
def onError(stage) {
...
def onFinal() {
  ...
  {
    lang: CODE_LANG.groovy,
    code: `import org.jenkinsci.plugins.pipeline.modeldefinition.Utils
...
def getImagesRepository() {
  ...
  {
    lang: CODE_LANG.groovy,
    code: `  def SHOULD_DEPLOY = isDeployBranch()
  if (SHOULD_DEPLOY) {
  ...
  {
    lang: CODE_LANG.groovy,
    code: `  def IMAGES_REPO = getImagesRepository()
  def PROJECT = getProject()
